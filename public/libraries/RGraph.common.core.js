...
//    };
    ...
    RGraph.drawTitle = function ()
    {
        ...
        if (args.object.type == 'bar' && prop.variant == '3d') {
            keypos = 'margin';
        }
    ...
    RGraph.Background.draw = function ()
    {
        ...
        var prop         = args.object.properties,
            ...
            args.object.context.fillStyle = prop.textColor;

            // If it's a bar and 3D variant, translate
            if (variant == '3d') {
                args.object.context.save();
                args.object.context.translate(prop.variantThreedOffsetx, -1 * prop.variantThreedOffsety);
            }
            ...
            if (
                ...
                && typeof prop.xaxisTitle === 'string'
                && prop.xaxisTitle.length
               ) {
                ...
                if (typeof prop.xaxisTitlePos === 'number') {
                    vpos = args.object.canvas.height - (marginBottom * prop.xaxisTitlePos);
                }
            ...
            if (
                ...
                && args.object.type !== 'gantt'
                && args.object.type !== 'scatter'
                && typeof prop.yaxisTitle === 'string'
                && prop.yaxisTitle.length) {
                ...
                } else if (args.object.type === 'hbar' && RGraph.isNull(prop.yaxisTitlePos) ) {
                    var yaxis_title_pos = prop.marginLeft - args.object.yaxisLabelsSize;
                } else {
                ...
                if (   prop.yaxisTitleAlign == 'right'
                    ...
                    || (args.object.type === 'hbar' && prop.yaxisPosition === 'right' && typeof prop.yaxisTitleAlign === 'undefined' && typeof prop.yaxisTitlePosition === 'undefined')
                   ) {

                    angle = 90;
                    yaxis_title_pos = typeof prop.yaxisTitlePos === 'number'
                                          ? (args.object.canvas.width - marginRight) + (prop.yaxisTitlePos * marginRight)
                                          : args.object.canvas.width - marginRight + (prop.yaxisLabelsSize || prop.textSize) + 10;
            ...
            args.object.context.beginPath();
                args.object.context.fillStyle   = prop.backgroundBarsColor1;
            ...
            args.object.context.beginPath();
                args.object.context.fillStyle   = prop.backgroundBarsColor2;
                args.object.context.strokeStyle = args.object.context.fillStyle;
    ...
    RGraph.drawBars = function ()
    {
        ...
        for (var i=0,len=hbars.length; i<len; ++i) {
            ...
            if (prop.xaxisPosition == 'center' && start == args.object.scale2.max && length < (args.object.scale2.max * -2)) {
            ...
            var w = args.object.canvas.width - prop.marginLeft - prop.marginRight;
...
//        //}
                    ...
                    if (coords && coords.length > 0) {
                        ...
                        args.object.context.beginPath();
                            ...
                            if ((typeof labels_processed[i] === 'object' && typeof labels_processed[i][1] === 'string')) {
                                ...
                                args.object.context.fillStyle = prop.labelsIngraphColor;
                            }
    ...
    RGraph.draw3DYAxis = function (args)
    {
        ...
        //if (!prop.noaxes && !prop.noyaxis) {
            ...
            if ( (args.object.type === 'hbar' || args.object.type === 'bar') && prop.yaxisPosition === 'center') {
                var x = ((args.object.canvas.width - marginLeft - marginRight) / 2) + marginLeft;
            } else if ((args.object.type === 'hbar' || args.object.type === 'bar') && prop.yaxisPosition === 'right') {
        ...
        if (typeof prop.backgroundImage === 'string') {
            ...
            if (typeof align === 'string') {
                if (align.indexOf('right') != -1) {
                    var x = args.object.canvas.width - (prop.backgroundImageW || img.width) - marginRight;
                } else {
                    var x = marginLeft;
                }
    
                if (align.indexOf('bottom') != -1) {
                    var y = args.object.canvas.height - (prop.backgroundImageH || img.height) - marginBottom;
                } else {
                    var y = marginTop;
                }
            } else {
            ...
            var oldAlpha = args.object.context.globalAlpha;
                args.object.context.globalAlpha = prop.backgroundImageAlpha;
    ...
    RGraph.Highlight.rect = function ()
    {
        ...
        if (prop.tooltipsHighlight) {
            ...
            args.object.context.beginPath();
                ...
                args.object.context.strokeStyle = prop.highlightStroke;
                args.object.context.fillStyle   = prop.highlightFill;
    ...
    RGraph.drawXAxis = function (obj)
    {
        var prop            = obj.properties,
            context         = obj.context,
            tickmarksLength = (typeof prop.xaxisTickmarksLength === 'number' ? prop.xaxisTickmarksLength : 3),
            isSketch        = (obj.type === 'bar' && prop.variant === 'sketch');
        ...
        //
        if (   typeof prop.xaxisLabels === 'object'
            && !RGraph.isNull(prop.xaxisLabels)
            && prop.xaxisLabels.length) {
            
            for (var i=0; i<prop.xaxisLabels.length; ++i) {
                if (typeof prop.xaxisLabels[i] === 'undefined' || prop.xaxisLabels[i] === null) {
                    prop.xaxisLabels[i] = '';
                }
            }
        }
        ...
        if ( (obj.type === 'hbar' || obj.type === 'gantt') && prop.xaxisPosition === 'bottom') {
            var y = obj.canvas.height - prop.marginBottom;
        } else if ( (obj.type === 'hbar' || obj.type === 'gantt') && prop.xaxisPosition === 'top') {
            var y = prop.marginTop;
        //} else if (obj.type === 'scatter' && prop.yaxisScaleInvert) {
        //    var y = obj.getYCoord(prop.yaxisScaleMin > 0 ? prop.yaxisScaleMin : 0);
        } else {
            var y = obj.getYCoord(prop.yaxisScaleMin > 0 ? prop.yaxisScaleMin : 0);
        }

        // Special case for a Line chart with an inverted scale
        if (obj.type === 'line' && prop.yaxisScaleInvert && prop.yaxisScaleMin === 0) {
            y = obj.getYCoord(obj.scale2.max);
        }

        // Special case for a Scatter chart with an inverted scale
        if (obj.type === 'scatter' && prop.yaxisScaleInvert) {
        ...
        if (obj.type === 'drawing.xaxis') {
            if (prop.xaxisPosition === 'center') {
                y = ((obj.canvas.height - prop.marginTop - prop.marginBottom) / 2) + prop.marginTop;
            } else {
                y = obj.y;
            }
        }
        ...
        if (prop.xaxis) {
            ...
            obj.path(
                'lw % b m % % l % % s %',
                prop.xaxisLinewidth,
                prop.marginLeft - (isSketch ? 5 : 0),
                
                y - (isSketch ? 2 : 0),
                
                obj.canvas.width - prop.marginRight + (isSketch ? 7 : 0), y + (isSketch ? 2 : 0),
                prop.xaxisColor
            );
                ...
                if (!isSketch) {
                    ...
                    if (prop.xaxisTickmarks) {
                        ...
                        if (typeof prop.xaxisTickmarksCount === 'number') {
                            var xaxisTickmarksCount = prop.xaxisTickmarksCount;
                        ...
                        // HBar - get number of tickmarks from the xaxisLabelsCount property and default to 5
                            ...
                            var xaxisTickmarksCount = (prop.xaxisLabelsCount || 5);
                        ...
                        } else if (obj.type === 'scatter' && prop.scale) {
                            var xaxisTickmarksCount = 5;

                            
                        // Scatter - with labels - get the number of tickmarks from the number of labels
                        } else if (obj.type === 'scatter' && prop.xaxisLabels) {
                            var xaxisTickmarksCount = prop.xaxisLabels.length;
                        ...
                        // Waterfall - get the number of tickmarks from the number of datapoints
                            ...
                            var xaxisTickmarksCount = obj.data.length + (prop.total ? 1 : 0);
                            
                        // Drawing API X axis
                        } else if (obj.type === 'drawing.xaxis') {

                            if (prop.scale) {
                                var xaxisTickmarksCount = prop.xaxisScaleLabelsCount;
                            } else if (prop.xaxisLabels && prop.xaxisLabels.length) {
                                var xaxisTickmarksCount = prop.xaxisLabels.length;
                            } else {
                                var xaxisTickmarksCount = 5;
                ...
                if (prop.xaxisPosition === 'center' && prop.yaxisScaleMin >= 0) {
                    if (prop.yaxisScaleInvert) {
                        var tickmarksYStart = obj.getYCoord(prop.yaxisScaleMax) - tickmarksLength;
                        var tickmarksYEnd   = obj.getYCoord(prop.yaxisScaleMax) + tickmarksLength;
                    } else {
                        var tickmarksYStart = obj.getYCoord(prop.yaxisScaleMin) - tickmarksLength;
                        var tickmarksYEnd   = obj.getYCoord(prop.yaxisScaleMin) + tickmarksLength;
                    }

                } else if (prop.xaxisPosition === 'center' || prop.yaxisScaleMin < 0) {
                    if (prop.yaxisScaleInvert) {
                    ...
                    if (prop.yaxisScaleMin < 0 && prop.yaxisScaleMax > 0) {
                        var tickmarksYStart = obj.getYCoord(0) - tickmarksLength;
                        var tickmarksYEnd   = obj.getYCoord(0) + tickmarksLength;
                    }
                    ...
                    if (obj.getYCoord) {
                        ...
                        var tickmarksYStart = prop.marginTop - tickmarksLength;
                        var tickmarksYEnd   = prop.marginTop;
                    }

                } else {

                    if (obj.getYCoord) {
                        if (obj.type === 'line' && prop.yaxisScaleInvert) {
                            ...
                            var tickmarksYStart = obj.getYCoord(prop.yaxisScaleMin);
                            var tickmarksYEnd   = obj.getYCoord(prop.yaxisScaleMin) + tickmarksLength;
                        }

                    } else {

                        var tickmarksYStart = obj.canvas.height - prop.marginBottom;
                        var tickmarksYEnd   = obj.canvas.height - prop.marginBottom + tickmarksLength;
                    }
                }
                ...
                for (var i=0; i<=xaxisTickmarksCount; ++i) {
                    ...
                    // told not to
                    if (RGraph.isNull(prop.xaxisTickmarksLastLeft)) {

                        if ( i === 0 && prop.yaxis && prop.yaxisPosition === 'left' && !(prop.xaxisScaleMax > 0 && prop.xaxisScaleMin < 0) ) {
                            continue;
                        }

                    } else if (i === 0 && !prop.xaxisTickmarksLastLeft) {
                        continue;
                    }
                    
                    // Don't draw the RIGHT tickmark if there's a Y axis on the right or if specifically
                    // told not to
                    if (RGraph.isNull(prop.xaxisTickmarksLastRight)) {
                        if (   i === xaxisTickmarksCount && prop.yaxis && prop.yaxisPosition === 'right') {
                            continue;
                        }
                    } else if ( i === xaxisTickmarksCount && !prop.xaxisTickmarksLastRight ) {
                        continue;
                    }
    
                    var x = (((obj.canvas.width - prop.marginLeft - prop.marginRight) / xaxisTickmarksCount) * i) + prop.marginLeft;

                    // If the chart is an HBar and the Y axis is in the center
                    // then don't draw a tickmark at the same position as the
                    // Y axis.
                    if (obj.type === 'hbar' ) {
                        if (prop.yaxisPosition === 'center' && x > obj.getXCoord(0) - 2 && x < obj.getXCoord(0) + 2) {
                            continue;
                        } else if (prop.yaxisPosition === 'left' && prop.xaxisScaleMin < 0 && prop.xaxisScaleMax > 0 && x > obj.getXCoord(0) - 2 && x < obj.getXCoord(0) + 2) {
                            continue;
                        }
                    }
        ...
        if (prop.xaxisScale) {
            ...
            // Get the scale for a drawing API X axis
                ...
                if (prop.xaxisScale) {
                    ...
                    scale = RGraph.getScale({object: this, options: {
                        ...
                        'scale.labels.count': prop.xaxisScaleLabelsCount,
                        'scale.strict':       true
                     }});

                    for (var i=0; i<=prop.xaxisScaleLabelsCount; ++i) {
                    
                        var original = (((prop.xaxisScaleMax - prop.xaxisScaleMin) / prop.xaxisScaleLabelsCount) * i) + prop.xaxisScaleMin;
                        var hmargin  = prop.marginInner;
                    
                        if (typeof prop.xaxisScaleFormatter === 'function') {
                            var text =  (prop.xaxisScaleFormatter)(this, original)
                        } else {
                    
                            text = RGraph.numberFormat({
                                object:    obj,
                                number:    original.toFixed(original === 0 ? 0 : prop.xaxisScaleDecimals),
            ...
            for (var i=0; i<scale.labels.length; ++i) {
                ...
                var section = (obj.canvas.width - prop.marginLeft - prop.marginRight) / scale.labels.length;
                
                if (prop.yaxisPosition === 'right') {
                    var x = prop.marginLeft + (section * i);
                } else if (prop.yaxisPosition === 'center') {
                    section /= 2;
                    var x = obj.getXCoord(0) + section + (section * i);
                } else {
                    var x = prop.marginLeft + (section * i) + section;
                }
                
                var y = prop.xaxisPosition === 'top' ? obj.marginTop - (prop.xaxisTickmarksLength || 0) + prop.xaxisLabelsOffsety - 5 : (obj.canvas.height - prop.marginBottom) + (prop.xaxisTickmarksLength || 0) + prop.xaxisLabelsOffsety + 5;
                
                if (obj.type === 'drawing.xaxis') {
                    if (prop.xaxisPosition === 'top') {
                        y = obj.y - (prop.xaxisTickmarksLength || 0) + prop.xaxisLabelsOffsety;
                    } else {
                        y = obj.y + (prop.xaxisTickmarksLength || 0) + prop.xaxisLabelsOffsety + 5;
                    }
                }
          ...
          textConfPrefix: 'xaxisLabels',
                    ...
                    y:      y,
                    text:   typeof prop.xaxisScaleFormatter === 'function' ? (prop.xaxisScaleFormatter)({object: obj, number: scale.values[i]}) : (prop.yaxisPosition === 'right' ? String(scale.labels[scale.labels.length - 1 - i]) : String(scale.labels[i])),
                    valign: typeof prop.xaxisLabelsValign === 'string' ? prop.xaxisLabelsValign : (prop.xaxisPosition === 'top' ? 'bottom' : valign),
                    halign: typeof prop.xaxisLabelsHalign === 'string' ? prop.xaxisLabelsHalign : halign,
                    marker: false,
                    angle:  angle,
                    tag:    'xaxis.labels'
                });
                ...
                //
                if (obj.type === 'hbar' && prop.yaxisPosition === 'center') {
                  ...
                  textConfPrefix: 'xaxisLabels',
                            x:      x + prop.xaxisLabelsOffsetx,
                            y:      prop.xaxisPosition === 'top' ? obj.marginTop - (prop.xaxisTickmarksLength || 0) + prop.xaxisLabelsOffsety - 5 : (obj.canvas.height - prop.marginBottom) + (prop.xaxisTickmarksLength || 0) + prop.xaxisLabelsOffsety + 5,
                            text:   typeof prop.xaxisScaleFormatter === 'function' ? (prop.xaxisScaleFormatter)({object: obj, number: scale.values[i]}) : '-' + String(scale.labels[i]),
                            valign: typeof prop.xaxisLabelsValign === 'string' ? prop.xaxisLabelsValign : (prop.xaxisPosition === 'top' ? 'bottom' : valign),
                            halign: typeof prop.xaxisLabelsHalign === 'string' ? prop.xaxisLabelsHalign : halign,
                            marker: false,
                            angle:  angle,
                ...
                RGraph.text({
                  object:   obj,
          textConfPrefix:   'xaxisLabels',
                    x:      prop.yaxisPosition === 'right' ? obj.canvas.width - prop.marginRight + prop.xaxisLabelsOffsetx : (prop.yaxisPosition === 'center' ? obj.getXCoord(0) + prop.xaxisLabelsOffsetx : prop.marginLeft + prop.xaxisLabelsOffsetx),
                    y:      y,
                    text:   typeof prop.xaxisScaleFormatter === 'function' ? (prop.xaxisScaleFormatter)({object: obj, number: 0}) : str,
                    valign: typeof prop.xaxisLabelsValign === 'string' ? prop.xaxisLabelsValign : (typeof prop.xaxisLabelsValign === 'string' ? prop.xaxisLabelsValign : (prop.xaxisPosition === 'top' ? 'bottom' : valign)),
                    halign: typeof prop.xaxisLabelsHalign === 'string' ? prop.xaxisLabelsHalign : halign,
                    marker: false,
                    angle:  angle,
                    tag:    'xaxis.labels'
        ...
        } else if (prop.xaxisLabels && prop.xaxisLabels.length && (prop.xaxisLabelsPosition === 'section' || prop.xaxisLabelsPosition === 'edge') ) {

            if (prop.xaxisLabelsPosition === 'edge') {
                var section = (obj.canvas.width - prop.marginLeft - prop.marginRight - (prop.marginInner || 0) - (prop.marginInner || 0) ) / (prop.xaxisLabels.length - 1);
            } else {
                var section = (obj.canvas.width - prop.marginLeft - prop.marginRight) / prop.xaxisLabels.length;
            }

            for (var i=0; i<prop.xaxisLabels.length; ++i) {

                if (prop.xaxisLabelsPosition === 'edge') {
                    var x = prop.marginLeft + (prop.marginInner || 0) + (section * i);
                } else {
                    var x = prop.marginLeft + (section * i) + (section / 2) ;
                }
                ...
                // along the X scale
                if (typeof prop.xaxisLabels[i] === 'object' && obj.type === 'scatter') {

                    var rightEdge = 0;
                    var align     = prop.xaxisLabelsSpecificAlign === 'left' ? 'left' : 'center';
                    var halign    = 'center'
                    

                    if (prop.xaxisLabels[i+1] && typeof prop.xaxisLabels[i+1][1] === 'number') {
                        rightEdge = prop.xaxisLabels[i+1][1];

                    } else {
                        rightEdge = prop.xaxisScaleMax;
                    }

                    var leftEdge = prop.xaxisLabels[i][1];
                   ...
                   object:      obj,
           textConfPrefix:      'xaxisLabels',
                        x:      x + 5 + prop.xaxisLabelsOffsetx,
                        y:      y + 5 + prop.xaxisLabelsOffsety,
                        valign: valign,
                        halign: angle != 0 ? 'right' : halign,
                        text:   String(prop.xaxisLabels[i][0]),
                        angle:  angle,
                        marker: false,
                        tag:    'labels.specific'
                    });
                    ...
                    obj.path('b m % % l % % s #bbb',
                        Math.round(prop.marginLeft + (((prop.xaxisLabels[i][1] - prop.xaxisScaleMin) / (prop.xaxisScaleMax - prop.xaxisScaleMin )) * (obj.canvas.width - prop.marginLeft - prop.marginRight) )), obj.canvas.height - prop.marginBottom,
                        Math.round(prop.marginLeft + (((prop.xaxisLabels[i][1] - prop.xaxisScaleMin) / (prop.xaxisScaleMax - prop.xaxisScaleMin))) * (obj.canvas.width - prop.marginLeft - prop.marginRight) ), obj.canvas.height - prop.marginBottom + 20
                    );
                    
                    // Draw the final indicator line if we're on the final label
                    if (i === prop.xaxisLabels.length - 1) {
                        obj.path('b m % % l % % s #bbb',
                            obj.canvas.width - prop.marginRight, obj.canvas.height - prop.marginBottom,
                            obj.canvas.width - prop.marginRight, obj.canvas.height - prop.marginBottom + 20
                        );
                    }
                ...
                // A regular label
                    ...
                    var y = prop.xaxisPosition === 'top' ? prop.marginTop + prop.xaxisLabelsOffsety - 5 : (obj.canvas.height - prop.marginBottom) + prop.xaxisLabelsOffsety + 5;
        ...
        if (prop.xaxisTitle) {
            ...
            var x = prop.marginLeft + ((obj.canvas.width - prop.marginLeft - prop.marginRight) / 2) + prop.xaxisTitleOffsetx;
            var y = prop.xaxisPosition === 'top' ? prop.marginTop  - 7 + prop.xaxisTitleOffsety - prop.xaxisTickmarksLength : obj.canvas.height - prop.marginBottom + 7 + prop.xaxisTitleOffsety + prop.xaxisTickmarksLength;
            
            if (obj.type === 'drawing.xaxis') {
                y = obj.y  + 7 + prop.xaxisTitleOffsety + prop.xaxisTickmarksLength;
            }

            
            // Get the size of the X axis labels
            if (prop.xaxisScale || (prop.xaxisLabels && prop.xaxisLabels.length) ) {
                var textConf = RGraph.getTextConf({
                    object: obj,
                    prefix: 'xaxisLabels'
                });
            ...
            if (typeof prop.xaxisTitlePos === 'number') {
                if (prop.xaxisPosition === 'top') {
                    y = prop.marginTop * prop.xaxisTitlePos;
                } else {
                    y = obj.canvas.height - prop.marginBottom + (prop.marginBottom * prop.xaxisTitlePos);
                }
            }

            // Specific X and Y coordinates for the title
            if (typeof prop.xaxisTitleX === 'number') x = prop.xaxisTitleX;
            if (typeof prop.xaxisTitleY === 'number') y = prop.xaxisTitleY;
            ...
            RGraph.text({
              ...
              textConfPrefix: 'xaxisTitle',
                x:      x,
                y:      y,
                text:   prop.xaxisTitle.toString(),
                valign: prop.xaxisPosition === 'top' ? (prop.xaxisTitleValign || 'bottom') : (prop.xaxisTitleValign || 'top'),
                halign: prop.xaxisTitleHalign || 'center',
                marker: false,
                tag:    'xaxis.title'
            });
        }
    ...
    RGraph.drawYAxis = function (obj)
    {
        var prop            = obj.properties,
            context         = obj.context,
            isSketch        = obj.type === 'bar' && prop.variant === 'sketch';
            tickmarksLength = typeof prop.yaxisTickmarksLength === 'number' ? prop.yaxisTickmarksLength : 3;

        // If drawing a HBar or a Gantt chart then set the yaxisLabelsSpecific option
        if (obj.type === 'hbar') {
            prop.yaxisLabelsSpecific = prop.yaxisLabels;
        }
        
        // Calculate the X coordinate for the Y axis
        if ( (obj.type === 'hbar' || obj.type === 'gantt') && prop.yaxisPosition === 'left') {
            var x = obj.getXCoord(0);
        
        } else if ( (obj.type === 'hbar' || obj.type === 'gantt') && prop.yaxisPosition === 'right') {
            var x = obj.canvas.width - prop.marginRight;
        
        } else if (obj.type === 'drawing.yaxis') {
            var x = obj.x;
        
        } else {
            var x = prop.yaxisPosition === 'right' ? obj.canvas.width - prop.marginRight : (obj.type === 'hbar' ? obj.getXCoord(0) : prop.marginLeft);
        }
        ...
        if (prop.yaxis) {
            ...
            obj.path(
                'lw % b m % % l % % s %',
                prop.yaxisLinewidth,
                x - (isSketch ? 5 : 0),     prop.marginTop - (isSketch ? 2 : 0),                
                x + (isSketch ? 7 : 0),     obj.canvas.height - prop.marginBottom  + (isSketch ? 2 : 0),
                prop.yaxisColor
            );
            ...
            if (!isSketch) {
                if (prop.yaxisTickmarks) {
                    ...
                    if (typeof prop.yaxisTickmarksCount === 'number') {
                        var yaxisTickmarksCount = prop.yaxisTickmarksCount;

                    // Bar - get number of tickmarks from the number of data points
                    } else if (obj.type === 'bar') {
                        var yaxisTickmarksCount = prop.yaxisLabelsSpecific ? prop.yaxisLabelsSpecific.length - 1 : prop.yaxisLabelsCount;
                    ...
                    // Scatter - with a scale - get the number of tickmarks from the number of scale labels
                        ...
                        var yaxisTickmarksCount = prop.yaxisLabelsCount;
                ...
                for (var i=0; i<=yaxisTickmarksCount; ++i) {
                    ...
                    // told not to
                    if (RGraph.isNull(prop.yaxisTickmarksLastTop)) {
                        if (i === 0 && prop.xaxis && prop.xaxisPosition === 'top') {
                            continue;
                        }
                    } else if (i === 0 && !prop.yaxisTickmarksLastTop) {
                        continue;
                    }
                    
                    // Don't draw the BOTTOM tickmark if there's an X axis at the bottom or if specifically
                    // told not to
                    if (RGraph.isNull(prop.yaxisTickmarksLastBottom)) {

                        if (i === yaxisTickmarksCount && prop.xaxis && prop.xaxisPosition === 'bottom') {
                            continue;
                        }
                    } else if (i === yaxisTickmarksCount && !prop.yaxisTickmarksLastBottom) {
                        continue;
                    }

                    var y           = (((obj.canvas.height - prop.marginTop - prop.marginBottom) / yaxisTickmarksCount) * i) + prop.marginTop;
                    ...
                    if (    prop.xaxis
                        && (prop.xaxisPosition === 'center' || (prop.xaxisPosition === 'bottom' && prop.yaxisScaleMin < 0 && prop.yaxisScaleMax > 0))
                        && y > (xaxisYCoord - 1)
                        && y < (xaxisYCoord + 1)) {
                        continue;
                    }
                    ...
                    if (prop.yaxisScaleMin < 0 && prop.yaxisScaleMax > 0) {
                        obj.path(
                            'b m % % l % % s %',
                            tickmarksXStart, obj.canvas.height - prop.marginBottom,
                            tickmarksXEnd,   obj.canvas.height - prop.marginBottom,
                            prop.yaxisColor
                        );
                    }
                    ...
                    if (obj.type === 'scatter' && prop.xaxisPosition === 'center' && prop.yaxisScaleInvert) {
                        obj.path(
                            'b m % % l % % s %',
                            tickmarksXStart,   prop.marginTop,
                            tickmarksXEnd,     prop.marginTop,
                            prop.yaxisColor
                        );
                    }
        ...
        // Draw a Y axis scale.
        //
        if (prop.yaxisScale && !prop.yaxisLabelsSpecific) {
            if (obj.type === 'drawing.yaxis') {
                if (prop.yaxisScale) {
                    ...
                    obj.scale2 = RGraph.getScale({object: obj, options: {
                        ...
                        'scale.decimals':     Number(prop.yaxisScaleDecimals),
                        ...
                        'scale.labels.count': prop.yaxisScaleLabelsCount,
                        'scale.strict':       true
                     }});

                    for (var i=0; i<=prop.yaxisScaleLabelsCount; ++i) {
                    
                        var original = (((prop.yaxisScaleMax - prop.yaxisScaleMin) / prop.yaxisScaleLabelsCount) * i) + prop.yaxisScaleMin;
                        var hmargin  = prop.marginInner;
                    
                        if (typeof prop.yaxisScaleFormatter === 'function') {
                            var text =  (prop.yaxisScaleFormatter)(this, original)
                        } else {
                    
                            text = RGraph.numberFormat({
                                object:    obj,
                                number:    original.toFixed(original === 0 ? 0 : prop.yaxisScaleDecimals),
            ...
            var scale           = obj.scale2;
            obj.maxLabelLength = Math.max(obj.maxLabelLength, obj.context.measureText(obj.scale2.labels[4]).width * 2);
            ...
            if (prop.xaxisPosition === 'center') {
                ...
                var halfHeight = ((obj.canvas.height - prop.marginTop - prop.marginBottom) / 2);
                
                // Draw the top halves labels
                for (var i=0; i<scale.labels.length; ++i) {

                    var section = (obj.canvas.height - prop.marginTop - prop.marginBottom) / (scale.labels.length * 2);
                    var y       = prop.marginTop + (section * i);

                    RGraph.text({
                      object:   obj,
              textConfPrefix:   'yaxisLabels',
                        x:      prop.yaxisPosition === 'right' ? x + (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx + 5 : x - (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx - 5,
                        y:      prop.yaxisScaleInvert ? halfHeight + prop.marginTop - (section * i) + prop.yaxisLabelsOffsety : prop.marginTop + (section * i) + prop.yaxisLabelsOffsety,
                        text:   String(scale.labels[scale.labels.length - 1 - i]),
                        valign: typeof prop.yaxisLabelsValign === 'string' ? prop.yaxisLabelsValign : valign,
                        halign: typeof prop.yaxisLabelsHalign === 'string' ? prop.yaxisLabelsHalign : (prop.yaxisPosition === 'right' ? 'left' : halign),
                        marker: false,
                        angle:  angle,
                        tag:    'yaxis.labels'
                    });
                ...
                for (var i=0; i<scale.labels.length; ++i) {
                    ...
                    if (i === 0 && prop.yaxisScaleInvert) continue;

                    var section = (obj.canvas.height - prop.marginTop - prop.marginBottom) / (scale.labels.length * 2);
                    var y       = prop.marginTop + ((obj.canvas.height - prop.marginTop - prop.marginBottom) / 2) + (section * i) + section;
    
                    RGraph.text({
                      object:   obj,
              textConfPrefix:   'yaxisLabels',
                        x:      prop.yaxisPosition === 'right' ? x + (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx + 5 : x - (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx - 5,
                        y:      prop.yaxisScaleInvert ? halfHeight + prop.marginTop + (section * i) : y + prop.yaxisLabelsOffsety,
                        text:   '-' + (prop.yaxisScaleInvert ? String(scale.labels[scale.labels.length - i - 1]) : String(scale.labels[i])),
                        valign: typeof prop.yaxisLabelsValign === 'string' ? prop.yaxisLabelsValign : valign,
                        halign: typeof prop.yaxisLabelsHalign === 'string' ? prop.yaxisLabelsHalign : (prop.yaxisPosition === 'right' ? 'left' : halign),
                        marker: false,
                        angle:  angle,
                ...
                RGraph.text({
                  object:   obj,
          textConfPrefix:   'yaxisLabels',
                    x:      prop.yaxisPosition === 'right' ? x + 5 + (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx : x - 5 - (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx,
                    y:      prop.yaxisScaleInvert ? prop.marginTop + prop.yaxisLabelsOffsety : prop.marginTop + ((obj.canvas.height - prop.marginBottom - prop.marginTop) / 2) + prop.yaxisLabelsOffsety,
                    text:   typeof prop.yaxisScaleFormatter === 'function' ? (prop.yaxisScaleFormatter)({object: this,number: 0,unitspre: prop.yaxisScaleUnitsPre,unitspost: prop.yaxisScaleUnitsPost,point: prop.yaxisScalePoint,thousand: prop.yaxisScaleThousand,formatter: prop.yaxisScaleFormatter}) : (prop.yaxisScaleUnitsPre || '') + prop.yaxisScaleMin.toFixed(prop.yaxisScaleDecimals).replace(/\./, prop.yaxisScalePoint) + (prop.yaxisScaleUnitsPost || ''),
                    valign: typeof prop.yaxisLabelsValign === 'string' ? prop.yaxisLabelsValign : valign,
                    halign: typeof prop.yaxisLabelsHalign === 'string' ? prop.yaxisLabelsHalign : (prop.yaxisPosition === 'right' ? 'left' : halign),
                    marker: false,
                    angle:  angle,
              ...
              textConfPrefix:   'yaxisLabels',
                        x:      prop.yaxisPosition === 'right' ? x + 5 + (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx : x - 5 - (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx,
                        y:      obj.canvas.height - prop.marginBottom + prop.yaxisLabelsOffsety,
                        text:   typeof prop.yaxisScaleFormatter === 'function' ? (prop.yaxisScaleFormatter)({object: this,number: 0,unitspre: prop.yaxisScaleUnitsPre,unitspost: prop.yaxisScaleUnitsPost,point: prop.yaxisScalePoint,thousand: prop.yaxisScaleThousand,formatter: prop.yaxisScaleFormatter}) : (prop.yaxisScaleUnitsPre || '') + prop.yaxisScaleMin.toFixed(prop.yaxisScaleDecimals).replace(/\./, prop.yaxisScalePoint) + (prop.yaxisScaleUnitsPost || ''),
                        valign: typeof prop.yaxisLabelsValign === 'string' ? prop.yaxisLabelsValign : valign,
                        halign: typeof prop.yaxisLabelsHalign === 'string' ? prop.yaxisLabelsHalign : (prop.yaxisPosition === 'right' ? 'left' : halign),
                        marker: false,
            ...
            //
                ...
                for (var i=0; i<scale.labels.length; ++i) {
                    ...
                    var section = (obj.canvas.height - prop.marginTop - prop.marginBottom) / scale.labels.length;
                    
                    // Account for inverting the scale
                    if (prop.yaxisScaleInvert) {
                        var y = obj.canvas.height - prop.marginBottom - (section * i) - section - section;
                    } else {
                        var y = prop.marginTop + (section * i);
                    }
              ...
              textConfPrefix:   'yaxisLabels',
                        x:      prop.yaxisPosition === 'right' ? x + (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx + 5 : x - (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx - 5,
                        y:      y + section + prop.yaxisLabelsOffsety,
            ...
            textConfPrefix:   'yaxisLabels',
                    x:      prop.yaxisPosition === 'right' ? x + 5 + (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx : x - 5 - (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx,
                    y:      prop.yaxisScaleInvert ? obj.canvas.height - prop.marginBottom + prop.yaxisLabelsOffsety : prop.marginTop + prop.yaxisLabelsOffsety,
                    text:   typeof prop.yaxisScaleFormatter === 'function' ? (prop.yaxisScaleFormatter)({object: this,number: 0,unitspre: prop.yaxisScaleUnitsPre,unitspost: prop.yaxisScaleUnitsPost,point: prop.yaxisScalePoint,thousand: prop.yaxisScaleThousand,formatter: prop.yaxisScaleFormatter}) : (prop.yaxisScaleUnitsPre || '') + prop.yaxisScaleMin.toFixed(prop.yaxisScaleDecimals).replace(/\./, prop.yaxisScalePoint) + (prop.yaxisScaleUnitsPost || ''),
                    valign: typeof prop.yaxisLabelsValign === 'string' ? prop.yaxisLabelsValign : valign,
                    halign: typeof prop.yaxisLabelsHalign === 'string' ? prop.yaxisLabelsHalign : (prop.yaxisPosition === 'right' ? 'left' : halign),
                    marker: false,
            ...
            //
                ...
                for (var i=0; i<scale.labels.length; ++i) {
                    ...
                    var section = (obj.canvas.height - prop.marginTop - prop.marginBottom) / scale.labels.length;
                    var y       = prop.marginTop + (section * (i + (prop.yaxisScaleInvert ? 1 : 0)) );
              ...
              textConfPrefix:   'yaxisLabels',
                        ...
                        text:   String(prop.yaxisScaleInvert ? scale.labels[i] : scale.labels[scale.labels.length - 1 - i]),
                        valign: typeof prop.yaxisLabelsValign === 'string' ? prop.yaxisLabelsValign : valign,
                        halign: typeof prop.yaxisLabelsHalign === 'string' ? prop.yaxisLabelsHalign : (prop.yaxisPosition === 'right' ? 'left' : halign),
                        marker: false,
                        angle:  angle,
                ...
                var zerolabel = RGraph.numberFormat({
                    object:    obj,
                    number:    prop.yaxisScaleMin.toFixed(prop.yaxisScaleDecimals),
                    unitspre:  prop.yaxisScaleUnitsPre,
                    unitspost: prop.yaxisScaleUnitsPost,
                    point:     prop.yaxisScalePoint,
                    thousand:  prop.yaxisScaleThousand
          ...
          textConfPrefix:   'yaxisLabels',
                    x:      prop.yaxisPosition === 'right' ? x + 5 + (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx : x - 5 - (prop.yaxisTickmarksLength || 0) + prop.yaxisLabelsOffsetx,
                    y:      prop.yaxisScaleInvert ? prop.marginTop : obj.canvas.height - prop.marginBottom + prop.yaxisLabelsOffsety,
                    text:   typeof prop.yaxisScaleFormatter === 'function'
                                ? (prop.yaxisScaleFormatter)({object: this,number: 0,unitspre: prop.yaxisScaleUnitsPre,unitspost: prop.yaxisScaleUnitsPost,point: prop.yaxisScalePoint,thousand: prop.yaxisScaleThousand,formatter: prop.yaxisScaleFormatter})
                                : zerolabel,
                    valign: typeof prop.yaxisLabelsValign === 'string' ? prop.yaxisLabelsValign : valign,
                    halign: typeof prop.yaxisLabelsHalign === 'string' ? prop.yaxisLabelsHalign : (prop.yaxisPosition === 'right' ? 'left' : halign),
                    marker: false,
        ...
        //
        } else if (prop.yaxisLabelsSpecific && prop.yaxisLabelsSpecific.length && (prop.yaxisLabelsPosition === 'section' || prop.yaxisLabelsPosition === 'edge') ) {

            var section        = (obj.canvas.height - prop.marginTop - prop.marginBottom) / (prop.yaxisLabelsSpecific.length - (prop.yaxisLabelsPosition === 'section' ? 0 : 1));
            obj.maxLabelLength = 0;

            for (var i=0; i<prop.yaxisLabelsSpecific.length; ++i) {

                var y = prop.marginTop  + (section * i) + (prop.yaxisLabelsPosition === 'section' ? section / 2 : 0);

                RGraph.text({
                  object:   obj,
          textConfPrefix:   'yaxisLabels',
                    x:      obj.type === 'drawing.yaxis'
                                ? (prop.yaxisPosition === 'right' ? obj.x + 7 + prop.yaxisLabelsOffsetx : obj.x - 5 + prop.yaxisLabelsOffsetx)
                                : (prop.yaxisPosition === 'right' ? x + prop.yaxisLabelsOffsetx + 5 : prop.marginLeft - 5 + prop.yaxisLabelsOffsetx),
                    y:      y + prop.yaxisLabelsOffsety,
                    text:   String(prop.yaxisLabelsSpecific[i] || ''),
                    valign: typeof prop.yaxisLabelsValign === 'string' ? prop.yaxisLabelsValign : 'center',
                    halign: typeof prop.yaxisLabelsHalign === 'string' ? prop.yaxisLabelsHalign : (prop.yaxisPosition === 'right' ? 'left' : 'right'),
                    marker: false,
                    tag:    'yaxis.labels'
                });

                obj.maxLabelLength = Math.max(obj.maxLabelLength, obj.context.measureText(String(prop.yaxisLabelsSpecific[i])).width * 2);
            }
        }
        ...
        if (prop.yaxisTitle) {
            ...
            //
            if (obj.type === 'gantt') {
                for (var i=0, maxLabelLength=0; i<prop.yaxisLabels.length;++i) {
                    maxLabelLength = Math.max(maxLabelLength, RGraph.measureText(prop.yaxisLabels[i])[0]);
                }
            } else if (obj.scale2 && obj.scale2.labels) {
                var maxLabelLength = RGraph.measureText(obj.scale2.labels[obj.scale2.labels.length - 1])[0];
            }
            ...
            if (
                    (obj.type === 'hbar' && prop.yaxisLabels && prop.yaxisLabels.length)
                 || (obj.type === 'drawing.yaxis' && prop.yaxisLabelsSpecific && prop.yaxisLabelsSpecific.length)
               ) {
                maxLabelLength = (function (labels)
                {
                    for (var i=0,max=0; i<labels.length; ++i) {
                        var dim = RGraph.measureText(labels[i]);
                        max     = Math.max(max, dim[0])
                    }
                    
                    return max;
                })(obj.type === 'drawing.yaxis' ? prop.yaxisLabelsSpecific : prop.yaxisLabels);
            }

            var x = prop.yaxisPosition === 'right' ? (obj.canvas.width - prop.marginRight) + 5 + maxLabelLength + 10 : prop.marginLeft - 5 - maxLabelLength - 10;
            var y = ((obj.canvas.height - prop.marginTop - prop.marginBottom) / 2) + prop.marginTop;
            
            if (obj.type === 'drawing.yaxis') {
                var x = prop.yaxisPosition === 'right'
            ...
            if (typeof prop.yaxisTitlePos === 'number') {
                if (prop.yaxisPosition === 'right') {
                    x = obj.canvas.width - (prop.marginRight * prop.yaxisTitlePos);
                } else {
                    x = prop.marginLeft * prop.yaxisTitlePos;
                }
            }

            // Specific X and Y coordinates for the title
            if (typeof prop.yaxisTitleOffsetx === 'number') x += prop.yaxisTitleOffsetx;
            if (typeof prop.yaxisTitleOffsety === 'number') y += prop.yaxisTitleOffsety;

            // Specific X and Y coordinates for the title
            if (typeof prop.yaxisTitleX === 'number') x = prop.yaxisTitleX;
            ...
            textConfPrefix: 'yaxisTitle',
                x:          x,
                y:          y,
                text:       prop.yaxisTitle.toString(),
                valign:     prop.yaxisTitleValign || 'bottom',
                halign:     prop.yaxisTitleHalign || 'center',
                marker:     false,
                accessible: false,
                angle: -45,
                angle:      prop.yaxisPosition === 'right' ? 90 : -90,
                tag:        'yaxis.title'
            });
        }
    };
